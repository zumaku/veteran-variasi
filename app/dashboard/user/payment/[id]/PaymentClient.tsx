...
export const PAYMENT_LOGOS: Record<string, string> = {
  ...
  "VA_PERMATA": "https://upload.wikimedia.org/wikipedia/id/4/48/PermataBank_logo.svg",
  "VA_CIMB": "https://upload.wikimedia.org/wikipedia/commons/3/38/CIMB_Niaga_logo.svg",
  "VA_BSI": "https://upload.wikimedia.org/wikipedia/commons/6/69/Bank_Syariah_Indonesia.jpg",
  "QRIS": "https://upload.wikimedia.org/wikipedia/commons/a/a2/Logo_QRIS.svg",
  "OTC_INDOMARET": "https://upload.wikimedia.org/wikipedia/commons/9/9d/Logo_Indomaret.png",
  "OTC_ALFAMART": "https://upload.wikimedia.org/wikipedia/commons/8/86/Alfamart_logo.svg",
  "CC_VISA": "https://upload.wikimedia.org/wikipedia/commons/5/5c/Visa_Inc._logo_%282021%E2%80%93present%29.svg",
  "CC_MASTERCARD": "https://upload.wikimedia.org/wikipedia/commons/2/2a/Mastercard-logo.svg",
  "CC_JCB": "https://upload.wikimedia.org/wikipedia/commons/4/40/JCB_logo.svg",
  "CC_AMEX": "https://upload.wikimedia.org/wikipedia/commons/f/fa/American_Express_logo_%282018%29.svg",
};
    ...
    if (isCancelled) {
      return (
        <div className="bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 px-4 py-2 rounded-full flex items-center gap-2 font-bold max-w-fit mx-auto print:bg-transparent print:text-black">
          ...
          Pesanan Dibatalkan {isExpired && order.status === "PENDING" && "(Kadaluarsa)"}
        </div>
      );
    }
    return (
    ...
    <div className="max-w-3xl mt-8 mx-auto pb-10 print:max-w-full print:p-0">
      ...
      <div className="bg-card border border-border/60 rounded-3xl p-6 sm:p-10 shadow-sm print:shadow-none print:border-none print:bg-white print:text-black">
        ...
        <div className="hidden print:block text-center mb-8 border-b pb-4">
          <h1 className="text-3xl font-black font-montserrat tracking-tight mb-2">VETERAN VARIASI</h1>
          <p className="text-sm text-gray-500">Nota Pembelian & Bukti Booking</p>
        </div>
        ...
        <div className="text-center space-y-4 mb-8">
          ...
          {isPending && (
            <div className="space-y-2 mt-4 print:hidden">
              <p className="text-muted-foreground text-sm">Selesaikan pembayaran dalam waktu</p>
              ...
              <p className="text-xs text-muted-foreground bg-muted inline-block px-3 py-1 rounded-full mt-2">
                Batas Akhir: {format(new Date(order.expiredAt), "dd MMM yyyy, HH:mm", { locale: id })}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-muted/30 rounded-2xl p-6 border border-border/50 print:bg-transparent print:border-gray-200">
            <h2 className="text-lg font-bold mb-4 font-montserrat flex items-center justify-between border-b border-border/50 pb-3 print:border-gray-200">
              ...
              <span className="text-muted-foreground text-sm font-medium">#{order.orderNumber}</span>
            </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-border/50 print:border-gray-200">
                  ...
                  <div className="flex items-center gap-3">
                    {order.car?.image ? (
                      <div className="w-10 h-10 rounded-md overflow-hidden bg-muted border print:border-gray-200 shrink-0 relative">
                        <Image src={order.car.image} alt={order.car.brand} fill className="object-cover" />
                      </div>
                    ) : (
                ...
                <div className="space-y-1">
                  ...
                  <p className="font-bold text-sm">
                    {order.bookingDate ? format(new Date(order.bookingDate), "dd MMMM yyyy", { locale: id }) : "-"} 
                ...
                <div className="flex items-center gap-3">
                  {PAYMENT_LOGOS[order.paymentToken || order.paymentMethod] && (
                    <div className="bg-white p-1.5 rounded-md border border-border/50 shrink-0 h-8 flex items-center justify-center print:border-gray-300">
                      <Image 
                        src={PAYMENT_LOGOS[order.paymentToken || order.paymentMethod]} 
                        alt="Logo" 
                        width={48} 
                        height={20} 
                  ...
                  <p className="font-bold text-lg">{(order.paymentToken || order.paymentMethod).replace(/_/g, " ")}</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-sm text-muted-foreground font-semibold mb-1 print:text-left print:sm:text-right">Total Tagihan</p>
            ...
            {isPending && (
              <div className="space-y-4 print:hidden">
                <div className="bg-muted/50 p-4 rounded-xl border border-border/60">
                  <p className="text-sm font-semibold mb-2 text-foreground/80">Kode Pembayaran / Virtual Account</p>
                  <div className="flex items-center justify-between bg-background p-3 rounded-lg border-2 border-border focus-within:border-[#FFB800] transition-colors">
                    <span className="font-mono text-xl font-bold tracking-widest">{paymentCode}</span>
                    <button 
                      type="button"
                ...
                <div className="bg-orange-50 dark:bg-orange-950/20 p-4 rounded-xl border border-orange-200 dark:border-orange-900/50 flex gap-3 text-orange-800 dark:text-orange-200">
                  ...
                  <div className="text-sm space-y-1">
                    ...
                    <ol className="list-decimal pl-4 space-y-1">
                      <li>Buka aplikasi Mobile Banking atau e-Wallet pendukung Anda.</li>
                      <li>Pilih menu Transfer atau Pembayaran Tagihan / Virtual Account.</li>
                      <li>Masukkan kode pembayaran <span className="font-mono font-bold bg-orange-100 dark:bg-orange-900/40 px-1 rounded">{paymentCode}</span>.</li>
                      <li>Pastikan nominal yang muncul sesuai, yaitu <span className="font-bold">{rupiahConverter(order.totalAmount)}</span>.</li>
                      <li>Selesaikan pembayaran. Status pesanan akan terupdate otomatis (simulasi).</li>
                    </ol>
                  </div>
                </div>
              </div>
